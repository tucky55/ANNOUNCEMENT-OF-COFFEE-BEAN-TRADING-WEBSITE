...
<div class="container">
  <div class="row">
    ...
    <div class="col-sm-4 ">
        ...
        <br>
        <a class="btn btn-outline-secondary" href="show_bill_detail.php?id=<?= $row['bill_id']?>" role="button">รายละเอียด</a>
      </div>
    </div>
<?php } ?>
  </div>
  ...
  <div class="row">
    <?php
        ...
        $sql= "SELECT * FROM billboard
               ...
               INNER JOIN member ON billboard.mem_id = member.mem_id
               WHERE billboard.bill_status = 1
               and bill_posttype = 'ซื้อ'"; // เพิ่มเงื่อนไขนี้';
        $result= mysqli_query($conn,$sql);
        while($row= mysqli_fetch_array($result)){  
    ?>
    <div class="col-sm-4 ">
